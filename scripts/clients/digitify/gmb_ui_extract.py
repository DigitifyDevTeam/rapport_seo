"""Digitify — automated GMB (same steps as DeepCleaning).
...
def main() -> int:
    ...
    out_dir = ROOT / "outputs" / "digitify" / month
    out_dir.mkdir(parents=True, exist_ok=True)
    period_start, period_end = _period_bounds(month)

    if not SESSION.is_file():
        print("Run first: python scripts/clients/digitify/gmb_ui_prepare.py")
        return 1
